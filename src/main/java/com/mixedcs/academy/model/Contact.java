...
@NamedQueries({
        @NamedQuery(name = "Contact.findOpenMessage",
                query = "SELECT c FROM Contact c WHERE c.status = :status"),
        @NamedQuery(name = "Contact.updateMessageStatus",
                query = "UPDATE Contact c SET c.status = ?1 WHERE c.contactId = ?2")
})
@NamedNativeQueries({
        @NamedNativeQuery(name = "Contact.findOpenMessageNative",
                query = "SELECT * FROM contact_message c WHERE c.status = :status"
                ,resultClass = Contact.class),
        @NamedNativeQuery(name = "Contact.findOpenMessageNative.count",
                query = "select count(*) as cnt from contact_message c where c.status = :status",
        ...
        @NamedNativeQuery(name = "Contact.updateMessageStatusNative",
                query = "UPDATE contact_message c SET c.status = ?1 WHERE c.contact_id = ?2")
})
